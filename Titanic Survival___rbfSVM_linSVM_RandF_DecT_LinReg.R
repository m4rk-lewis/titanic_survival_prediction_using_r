---
  title: "Titanic Survival Prediction Using Radial Basis Function Support Vector Machines, Linear Support Vector Machines, Random Forest, Decision Tree, Linear Regression"
...
1)	Randomly select 80% of the observations in the titanic3 dataset to create a training dataset, then use the remaining observations to create a testing dataset.
...
2)	Use the created training dataset to conduct a 10-fold cross validation to evaluate the predictive accuracy of 5 different classification algorithms introduced in this module, i.e. logistic regression, decision tree, random forests, linear kernel-based svm and rbf kernel-based svm. Define set.seed(1) and use default parameters of those corresponding r functions.
...
```
#The rbf kernel-based svm is the optimal classification algorihtm since it obtains the highest predictive accuracy (i.e. 81.1%) over the 10-fold cross validation.


3)	Use the entire training dataset to retrain the best classification algorithm found by the cross-validation, then report the predictive accuracy of the retrained model on the testing dataset.
